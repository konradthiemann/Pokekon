...
2. **Phase B — Aggregierte KI-Insights:** LLM bekommt nicht einen Log, sondern die aggregierten Kennzahlen aus Abschnitt 3 und formuliert konkrete Listen-/Spielempfehlungen („gegen das aktuelle Feld -1 Karte X, +1 Y, weil…").
   ✅ *Umgesetzt (2026-09):* `POST /api/analysis/deck/:deckId` nutzt die **gleiche provider-agnostische Adapter-Infrastruktur** wie Phase A. Input ist eine geschlossene Fakten-Liste (Field-Score, Matchups, Card Deltas, Equilibrium Signals), Grounding ist **strukturell + richtung** (Aussage referenziert Fakt-ID, Vorzeichen passt zum Konfidenzband). Zahlen sind Platzhalter (Server rendert). Ergebnis wird mit Fakten-Snapshot in `deck_synthesis` gecacht; Cache-Schlüssel ist Hash über kanonisierte Fakten + Sprache + Prompt-Version.
...
- ~~**Matchup-Daten**: TrainerHill-CSV vs. eigene Matrix aus `tournament_standings`.~~ → **Entschieden (2026-08, revidiert): eigene Online-Bo1-Matrix aus `/pairings` + TrainerHill als Fallback**. Der Sync-Job fetcht `/pairings` pro Turnier und leitet Kopf-an-Kopf-Zählungen in `tournament_matchups` ab. `GET /api/meta/matchups?days&online&bo1` gibt eine gemischte Matrix: eigene Paare mit ausreichend Spielen (≥ `MIN_MATCHUP_GAMES`) überschreiben die TrainerHill-Daten; alles andere fällt auf TrainerHill zurück. Die Antwort enthält `matchupSource` (eigene vs. Fallback-Paare), damit das UI Echtdaten von Näherungswerten kennzeichnen kann.
