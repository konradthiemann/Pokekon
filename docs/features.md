...
| Archetype recommendation (ranked decklist clusters, KI-text, Global/Lokal-Umschalter) | Meta → Archetype drilldown | Auto on load (GET); user triggers generation |
...
**Page:** `DeckPage` — "Tips" section ("List Comparison" subsection)
...
**Page:** `DeckPage` — "Tips" section (primary panel)
...
visibility at zero logs — `OverviewPage` and `DeckPage` now show a
...
Unlike the three dashboard tabs (which are Zustand `activeTab` state, not URLs), these are real paths. `App.tsx` checks `legalDocForPath(window.location.pathname)` **before the login gate** — like `/reset-password` — so the pages are reachable while signed out (a legal requirement) and survive a reload via the API's SPA fallback.
...
**Layout (UI/UX redesign, 2026-09-23 — WIP on `design/frontend-uiux-overhaul`, not yet on `main`):** the KPI header, trend chips and window control below stay always visible; everything else sits behind a `SegmentedTabs` bar (`apps/web/src/components/shared/SegmentedTabs.tsx`, the same reusable component `DeckPage.tsx`'s Deck/Analytics/Tips switcher now also uses) with four tabs — **Matchups** (field performance + preparation panel + the full matchup table, collapsed by default behind a counted `data-testid="archetype-matchups-full-toggle"`-style "Vollständige Liste (N)" button so the curated preparation panel and the exhaustive table don't both render at once), **Empfehlung** (`ArchetypeRecommendationPanel`), **Turnier-Liste** (`TournamentBestListPanel`, only offered once ≥1 tournament is known), **Decklisten** (the raw decklist cards). Previously all seven sections rendered as separate full-width cards stacked vertically on one page — reworked after Konrad reported "so viele Listen und Erklärungen und so viel Scroll", following researched patterns from 17Lands/TrainerHill/Untapped.gg/op.gg (segmented control over stacked panels; a curated top-N list with a counted, explicit "show all" escape hatch instead of an always-rendered full list; sample size next to a percentage instead of prose).
...
- **Preparation panel:** opponents weighted by *frequency × matchup weakness* — common **and** bad-for-you decks rank first ("Darauf musst du vorbereitet sein"), plus the mirror probability and the good matchups (free wins). Each entry shows its own confidence band (as a compact `n={{games}}` sample-size label under the win-rate badge, the raw interval moved to a `title` tooltip rather than sitting in the row as text); entries whose interval still includes 50 % (`significant === false`) carry a `CircleHelp` icon + tooltip (previously a long inline "unsicher — Intervall schließt 50 % ein" text badge, which wrapped and visually overlapped the win-rate badge at moderate widths) and sort behind significant entries at the same weight tier, rather than disappearing. Both the threats and free-wins lists are capped to the top 5 (already weight-sorted) with a counted "+N weitere" expand link — the underlying `FieldScore` still carries every covered opponent, none of it is dropped, just not rendered until asked for.
- **Matchups vs the field:** a table (`MatchupTable`) listing this archetype's record against every covered opponent — win rate, its 95 % interval, game count, and meta share — sorted from most favourable to least favourable. Data comes from the same real online-Bo1 blend (`tournament_matchups` + TrainerHill fallback) as the matrix. Opponent icons are data-driven from `iconsById` returned by `GET /api/meta/archetypes/:id/analysis`. Lives inside the drilldown's "Matchups" tab, collapsed by default behind a "Vollständige Liste (N)" toggle beneath the preparation panel — it repeats the same underlying threats/free-wins data as one long list, so it stopped always-rendering alongside the curated panel above it.
...
- **Archetype recommendation (`ArchetypeRecommendationPanel`, Spec 10 Slice C UI, between the matchup table and the raw decklists):** a ranked decklist-cluster list (`RankedCluster[]`, already sorted by `rank`) plus an optional KI-text recommendation, backed by `GET`/`POST /api/analysis/archetype/:archetypeId`. Three toggle chips, "Global"/"Lokal"/"Mein Spielstil" (a `mode` state, not a fourth backend `scope`), re-fetch the ranking on click. Each cluster shows its rank, Wilson-lower-bound win rate + interval, average placement percentile (when available), W/L/T record, member-list count, a compact Pokémon-card preview of the representative list, and (behind a `data-testid="archetype-recommendation-cluster-decklist-toggle"` `<details>` disclosure, since the preview only ever showed Pokémon and had no way to see the rest besides an unusable-on-touch hover tooltip) the full 60-card decklist grouped Pokémon/Trainer/Energy via `DecklistCard.tsx`'s exported `CardGroup`. A `data-testid="archetype-recommendation-methodology"` `<details>` disclosure (collapsed by default, above the cluster list) spells out in plain language how rank/interval/record/placement/field-score are actually derived — added after Konrad pointed out the numbers had no explanation attached. The KI-text half follows the exact same state machine as `DeckSynthesisPanel` (§ "Deck synthesis" above) — cold start / missing key (+ demo ephemeral-token path) / stale badge / honest empty / error — generation is **user-triggered only**, a mode switch never auto-regenerates. Deliberately uses its own local component state (request-key pattern, like `ArchetypeDetail` itself) rather than the `dashboardStore`. **"Mein Spielstil" (Spec 10 Slice E UI, completing the backend from § "Archetype synthesis" below):** sends `scope: 'local'` + `usePersonalPrior: true` + a `personalRecord` derived from the `ArchetypeStats` entry matching `archetypeId` (same Limitless-slug identifier space) — `archetypeStats` is passed down `MetaPage` → `ArchetypeDetail` → this panel as a prop, so the panel itself stays store-free. Below the shared `DEFAULT_MIN_OWN_GAMES` (5) threshold (same one the server silently applies), a `data-testid="archetype-recommendation-personal-insufficient-data"` hint explains that the recommendation currently matches the local mode, without duplicating the server's threshold value. **Local field weighting (Spec 10 Slice D UI, completing the backend `localField` field described in § "Archetype Synthesis" below):** in "Lokal" AND "Mein Spielstil" mode (both are `scope: 'local'` server-side), the panel derives a `localField: LocalFieldEntry[]` from the shared `localMeta: string[]` store slice (§11 `LocalMetaPanel`) — same derivation `PredictionPanel.tsx` uses (now shared via `localFieldWeight.ts`'s `seedWeight`), resolving each archetype NAME to its real `archetypeId` via the current online meta (`archetypes`, an `ArchetypeRecommendationPanel` prop passed down `MetaPage` → `ArchetypeDetail`) and weighting it by a stored override (`getLocalMetaWeightOverrides()`, read once on mount) or, by default, `seedWeight(sharePct)`. Sent as `localField` on both GET and POST whenever `localField.length > 0`; never sent in "Global" mode, even with a non-empty local field. When `localMeta` is empty, a `data-testid="archetype-recommendation-local-field-empty"` hint explains that "Lokal"/"Mein Spielstil" currently match the global ranking. Each cluster whose `fieldScore` the server actually re-ranked (regardless of the panel's current `mode`, the field's presence is itself the signal) renders an extra `data-testid="archetype-recommendation-cluster-field-score"` line with the field-weighted win rate + interval band, alongside the existing Wilson-based one.
- **Per-tournament best list (`TournamentBestListPanel`, Spec 10 AC-G third bullet, `specs/archetype-meta-analysis.md` — after the raw decklists, conceptually separate from the KI recommendation above):** answers "which list of this archetype would have performed best at **this one, specific** tournament", backed by the read-only `GET /api/analysis/tournament/:tournamentId/archetype/:archetypeId` — never an LLM call. A `<select>` lists every tournament this archetype had a published decklist at in the window, from the dedicated `GET /api/meta/archetypes/:archetypeId/tournaments` (its own web-client `getArchetypeTournaments`, `apps/web/src/lib/api.ts`) — **not** derived from the paginated decklists list (§ "Most successful decklists" above), which used to be the source and silently only ever offered whichever page of decklists happened to already be loaded (Konrad reported the picker looking "veraltet"/stale). Fetched in its own `useEffect`/failure domain in `ArchetypeDetail.tsx`, deliberately separate from the main analysis+lists `Promise.all` — a failure or an older, not-yet-deployed API here (verified locally: the endpoint 404s against the current production API until this change ships) just omits the "Turnier-Liste" tab entirely rather than failing the whole drilldown. Picking a tournament clusters that archetype's decklists from **only that tournament's own standings** (reusing the exact same `clusterDecklists`/`rankClusters`/`computeClusterFieldScores`/`reorderClustersByFieldScore` composition Slice C/D already use for a multi-tournament window, just scoped to one tournament — no new `packages/shared` functions needed) and re-ranks them by their field-weighted score against **that tournament's actual field** (derived from its own standings, not a window aggregate). Each cluster renders the same rank/Wilson-interval/field-score/record/member-count/card-preview-plus-full-decklist-disclosure layout as the recommendation panel's clusters (own `data-testid="tournament-best-list-cluster-decklist-toggle"`), but as its own independent component (not shared, since the two features are conceptually distinct: multi-tournament KI text vs. single-tournament pure computation), including its own `data-testid="tournament-best-list-methodology"` disclosure with tournament-scoped wording (the field score here is always vs. this one event's field, never a window). The **evidence** for the top-ranked cluster reuses `ListFieldPerformance` (§16) with that cluster's aggregated `matchResults` against the response's `field` (the tournament's own archetypes, so opponent names resolve without a second request) — real per-opponent win/loss chips, not generated text. An archetype not played at the selected tournament renders an explicit empty state (`clusters: []`, "honestly empty", not an error); an unknown tournament id 404s (a bad parameter, not a legitimately empty result, unlike an unknown archetype).
...
requesting user; a `'global'` row is shared across all users).

**Real field-reweighting (Spec 10 Slice D, UI in § 15 `ArchetypeRecommendationPanel`):** an
optional `localField: { archetypeId, name, weight }[]` (same shape as the
Prediction panel's local field, § 11 `LocalMetaPanel`/`PredictionPanel`) on both GET (as a
JSON-encoded query param, `localField=<encodeURIComponent(JSON.stringify(...))>` — query strings
have no native array encoding) and POST (a real JSON array in the body). Only takes effect for
`scope: 'local'` with a non-empty `localField` — `scope: 'global'`, or `scope: 'local'` without a
`localField`, still rank the exact same clusters as before (backward compatible, no behaviour
change for existing callers). When active, each cluster's game-by-game `matchResults` (aggregated
across every member standing, Spec 10 Slice A/D) are turned into a per-opponent-archetype
`MatchupCell[]` and scored against `localField` via `computeFieldScores` (`fieldWinRate.ts`, Spec
3 — reused verbatim, not re-derived: `clusterFieldScore.ts` injects one synthetic zero-share
subject per cluster into the shares list, so the cluster can run through the exact same
field-weighting math as a real archetype). Clusters are then re-ranked by this field-weighted
score (`fieldWinRateLowPct`, same "lower bound protects against a small sample" principle as the
Wilson-based default ranking) instead of the plain Wilson lower bound; a cluster with no recorded
games against `localField` sinks to the end rather than being dropped. GET accepts the same
`localField` shape as POST so `currentInputHash` stays consistent between a field-weighted POST
and a matching GET (same reasoning as the personalisation fields below — otherwise GET would
wrongly report `stale: true` right after generating a field-weighted synthesis).
