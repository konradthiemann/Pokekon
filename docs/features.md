...
**Real field-reweighting (Spec 10 Slice D, backend only — no `apps/web` UI yet, follow-up
session):** an optional `localField: { archetypeId, name, weight }[]` (same shape as the
