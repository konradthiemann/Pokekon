...
function wrColor(v: number | null): string {
  ...
  return v >= 55 ? 'text-emerald-700' : v >= 45 ? 'text-amber-700' : 'text-red-700';
}
  ...
  return (
    <div className="card p-0 overflow-hidden">
      <div className="px-4 py-3 border-b border-slate-200 flex items-center gap-2 flex-wrap">
        ...
        <span className="text-xs text-slate-500">{t('analytics.turnQuality.subtitle')}</span>
        <div
      ...
      <div className="p-4">
        {loading ? (
          ...
          <p className="text-xs text-red-700 py-6 text-center">
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center gap-2 text-xs text-slate-600">
              <Swords className="w-3.5 h-3.5 text-slate-500" aria-hidden="true" />
                  ...
                  {data.prizeCurveWins
                    ...
                    .map((p) => (
                      <div key={p.turn} className="flex items-center gap-2 text-xs">
                        ...
                        <span className="w-8 shrink-0 text-right tabular-nums text-slate-700">
                          {p.avgPrizesRemaining.toFixed(1)}
                        </span>
                        <span
                          className="w-6 shrink-0 text-right text-slate-400"
