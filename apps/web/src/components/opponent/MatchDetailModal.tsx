...
function ActionBadge({ action }: { action: 'add' | 'remove' | 'increase' | 'decrease' }) {
  ...
  const map = {
    ...
    remove: 'bg-red-100 text-red-800 border border-red-200',
    increase: 'bg-blue-100 text-blue-800 border border-blue-200',
    decrease: 'bg-orange-100 text-orange-800 border border-orange-200',
  };
  return (
...
function PlayCard({ item }: { item: BattleAnalysisPlay }) {
  ...
  return (
    <div className="bg-slate-100 rounded-lg border border-slate-200 overflow-hidden">
      <button
        className="w-full flex items-start justify-between gap-3 px-3 py-2.5 text-left hover:bg-slate-100 transition-colors"
        onClick={() => setOpen((v) => !v)}
      >
  ...
  return (
    <div className="space-y-5">
      ...
      {analysis.deckSuggestions.length > 0 && (
        <section>
          <h4 className="flex items-center gap-1.5 text-xs font-semibold text-green-700 uppercase tracking-wide mb-2">
          ...
          {activeTab === 'log' && (
            <div className="space-y-3">
              ...
              <textarea
                ...
                rows={8}
                className="w-full bg-white border border-slate-300 rounded-xl px-3 py-2.5 text-xs text-slate-800 placeholder:text-slate-400 focus:outline-none focus:border-brand-500 focus:ring-2 focus:ring-brand-200 resize-none font-mono leading-relaxed"
              />
