...
Fokus auf wenige, gut belegte Punkte. Qualität vor Quantität.`;
  ...
  analysis.keyMoments = (analysis.keyMoments ?? []).filter((m) =>
    evidenceExistsInLog(m.evidence, log),
  );
  analysis.playMistakes = (analysis.playMistakes ?? []).filter((m) =>
    evidenceExistsInLog(m.evidence, log),
  );
  analysis.cardNotes = (analysis.cardNotes ?? []).filter((n) =>
    evidenceExistsInLog(n.evidence, log),
  );
  analysis.deckSuggestions = (analysis.deckSuggestions ?? []).filter((s) =>
