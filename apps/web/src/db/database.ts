    ...
    this.version(3)
      ...
      .upgrade((trans) => {
        ...
        return trans
          ...
          .then(() => trans.table('deckCards').toCollection().modify({ deckId: 1 }))
          .then(() => trans.table('deckSnapshots').toCollection().modify({ deckId: 1 }))
          .then(() => trans.table('opponentLogs').toCollection().modify({ deckId: 1 }));
      });
  }
}
