...
function detectPlayers(lines: string[]): [string, string] {
  ...
  const stop = new Set([
    'eine', 'einen', 'einem', 'einer', 'die', 'der', 'das', 'den', 'dem',
    'ist', 'hat', 'auf', 'von', 'mit', 'zu', 'in', 'aus', 'nach', 'bei',
    'bis', 'für', 'durch', 'gegen', 'ohne', 'um', 'an', 'jetzt',
    'Schadensaufteilung', 'Grundschaden', 'Gesamtschaden', 'Schwäche',
    'Pokémon', 'Karte', 'Karten', 'Basis', 'Enthüllte',
  ]);
...
function parseTurnBlock(
  ...
  for (const line of blockLines) {
    ...
    if (m && (m[1] === p1 || m[1] === p2)) { player = m[1]; break; }
    const m2 = line.match(/^.+ von (\S+) hat .+ eingesetzt/);
    if (m2 && (m2[1] === p1 || m2[1] === p2)) { player = m2[1]; break; }
  }
  ...
  return { turnNumber, player, cardsPlayed, damageDealt, kos, prizesGained, energyAttached, actionsCount };
}
...
export function parseBattleLog(log: string, myPlayerName: string): ParsedBattleLog {
  ...
  const player1 = myPlayerName && (myPlayerName === autoP1 || myPlayerName === autoP2)
    ? myPlayerName
    : autoP1;
  ...
  const turns = turnBlocks.map((block, i) =>
    parseTurnBlock(block, i + 1, player1, player2),
  );

  // ── Prize progression ────────────────────────────────────────────────────
  const prizes: Record<string, number> = { [player1]: 6, [player2]: 6 };
  const prizeProgression: PrizePoint[] = [
