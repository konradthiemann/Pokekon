...
export async function fetchRecentTournaments(
  ...
  const res = await limitlessFetch('/api/tournaments?game=PTCG&completed=true&limit=100&format=standard');
  ...
  for (const t of eligible.slice(0, 15)) { // cap at 15 to avoid too many requests
    try {
      ...
      const archMap = new Map<string, { displayName: string; wins: number; losses: number; count: number }>();
 ...
 */
export async function syncLiveMeta(
  onProgress?: (msg: string) => void,
): Promise<MetaSyncResult> {
  onProgress?.('Fetching recent tournaments…');

  const res = await limitlessFetch('/api/tournaments?game=PTCG&completed=true&limit=50&format=standard');
  if (!res.ok) throw new Error(`Tournament list fetch failed: ${res.status}`);
  ...
  type ArchStat = { displayName: string; wins: number; losses: number; ties: number; playerCount: number };
  ...
  for (const t of eligible) {
    try {
      ...
      for (const player of standings) {
        ...
        const existing = archMap.get(id) ?? { displayName, wins: 0, losses: 0, ties: 0, playerCount: 0 };
