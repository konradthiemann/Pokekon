...
export function computeDeckPerformanceStats(
  ...
  const lowActivityTurnRate = totalMyTurns > 0
    ? Math.round((lowActivityTurns / totalMyTurns) * 100)
    : 0;

  // ── Card performance ─────────────────────────────────────────────────────────
  const cardMap = new Map<string, { totalPlays: number; gamesPlayed: number; wins: number; losses: number }>();
