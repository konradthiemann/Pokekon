...
import type { Deck, DeckCard, DeckSnapshot, OpponentLog, MetaSnapshot, ArchetypeStats, DeckVariantStats } from '../types';
...
export async function deleteDeck(id: number): Promise<void> {
  await db.transaction('rw', db.decks, db.deckCards, db.deckSnapshots, db.opponentLogs, async () => {
...
export async function upsertDeckCard(card: Omit<DeckCard, 'id'>, deckId?: number): Promise<number> {
  ...
  const existing = deckId !== undefined
    ? await db.deckCards.where('deckId').equals(deckId).and(c => c.name === card.name).first()
 ...
 */
export async function saveDeckSnapshot(label: string, cards: DeckCard[], deckId?: number): Promise<number> {
...
export function parseDeckSnapshot(snap: DeckSnapshot): DeckCard[] {
  try { return JSON.parse(snap.cards); } catch { return []; }
}
...
export async function updateOpponentLog(id: number, patch: Partial<Omit<OpponentLog, 'id'>>): Promise<void> {
...
export async function getDeckVariantStats(decks: Deck[]): Promise<DeckVariantStats[]> {
  ...
  return decks.map((deck) => {
    ...
    const metaScore = totalMetaFreq > 0
      ? Math.round(encountered.reduce((s, m) => s + m.metaFreq * m.winRate, 0) / totalMetaFreq)
      : 0;

    const recentForm = logs
      .slice(0, 10)
      .map((l) => l.result as import('../types').MatchResult);
...
export async function getSnapshotStatsForArchetype(
  archetype: string,
): Promise<{ snapshotId: number | null; label: string; wins: number; losses: number; ties: number }[]> {
  const logs = await db.opponentLogs
    .where('archetype')
    .equals(archetype)
    .toArray();
  ...
  return [...grouped.entries()].map(([snapshotId, stat]) => ({
    snapshotId,
    label: snapshotId != null ? (snapMap.get(snapshotId) ?? `Snapshot #${snapshotId}`) : 'Untagged matches',
    ...stat,
  }));
}
