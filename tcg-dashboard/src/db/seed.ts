...
export async function seedIfEmpty(): Promise<void> {
  ...
  if (cardCount === 0) {
    await db.deckCards.bulkAdd(
      (SEED_DECK_CARDS as any[]).map((c: any) => ({ ...c, deckId: defaultDeckId }))
    );
  }
  if (logCount === 0) {
    await db.opponentLogs.bulkAdd(
      (SEED_OPPONENT_LOGS as any[]).map((l: any) => ({ ...l, deckId: defaultDeckId }))
    );
  }
  if (metaCount === 0) await db.metaSnapshots.bulkAdd(SEED_META_SNAPSHOTS as any);
