...
import type { ArchetypeStats, DeckCard, DeckRecommendation, DeckSnapshot, DeckPerformanceStats, OpponentLog } from '../types';
...
const TECH_SUGGESTIONS: Record<string, { card: string; reason: string }> = {
  'Dragapult ex':           { card: 'Eri',             reason: 'Hand disruption (H regulation) slows Dragapult\'s setup; reduces opponent hand mid-game.' },
  'Dragapult Blaziken':     { card: 'Eri',             reason: 'Disrupts the Blaziken energy acceleration before it comes online.' },
  'Dragapult Dusknoir':     { card: 'Unfair Stamp',    reason: 'Resets opponent\'s hand to a small number before Dusknoir\'s Ability chain activates.' },
  "N's Zoroark ex":         { card: 'Briar',           reason: 'Snipes benched Zorua before it evolves, cutting off the draw engine.' },
  "N's Zoroark":            { card: 'Briar',           reason: 'Snipes benched Zorua before it evolves, cutting off the draw engine.' },
  'Lucario Hariyama':       { card: 'Fezandipiti ex',  reason: 'Disrupts the bench and hand to slow Lucario\'s aggressive early game.' },
  'Alakazam Dudunsparce':   { card: 'TM: Devolution',  reason: 'Devolves Alakazam back to Abra, removing the ability engine.' },
  'Starmie Froslass':       { card: 'Eri',             reason: 'Hand disruption breaks the Starmie / Froslass setup loop.' },
  "Cynthia's Garchomp ex":  { card: 'Unfair Stamp',    reason: 'Reduces hand size before Garchomp\'s ability chain activates.' },
  "Rocket's Mewtwo":        { card: 'Eri',             reason: 'Hand disruption prevents setting up the Mewtwo attack chain.' },
  'Ogerpon Meganium':       { card: 'Fezandipiti ex',  reason: 'Disrupts the Meganium bench, preventing energy acceleration for Ogerpon.' },
  'Raging Bolt Ogerpon':    { card: 'Hero\'s Cape',    reason: 'Reduces damage taken from Raging Bolt\'s high-damage attacks.' },
  'Raging Bolt ex':         { card: 'Hero\'s Cape',    reason: 'Reduces damage from Raging Bolt\'s Ancient attacks.' },
  'Grimmsnarl Froslass':    { card: 'TM: Devolution',  reason: 'Devolves Grimmsnarl to interrupt ability-based strategies before they lock in.' },
  "Rocket's Honchkrow":     { card: 'Eri',             reason: 'Hand disruption to break Honchkrow\'s setup before it attacks.' },
  'Okidogi Barbaracle':     { card: 'Unfair Stamp',    reason: 'Hand reset disrupts Barbaracle\'s lock strategy from the start.' },
  'Mega Venusaur':          { card: 'Briar',           reason: 'Bench damage knocks out the support Pokémon enabling Mega Venusaur.' },
  'Greninja':               { card: 'Unfair Stamp',    reason: 'Hand disruption slows the Greninja setup chain.' },
  'Slowking':               { card: 'TM: Devolution',  reason: 'Devolves Slowking to interrupt ability-based stall strategies.' },
};
  ...
  return useMemo(() => {
    ...
    if (!bossCard) {
      recs.push({
        ...
        reasoning: "Gust effect is a staple. Missing it limits ability to take prizes around tanky benched Pokémon.",
        dataPoints: 0,
      });
    ...
    if (deckSnapshots.length >= 2 && opponentLogs.length >= 6) {
      ...
      const snapStats = snapIds.map((id) => {
      ...
      if (snapStats.length >= 2) {
        ...
        if (currentSnap && bestSnap.id !== currentSnap.id && bestSnap.wr - currentSnap.wr >= 15) {
          const bestLabel = deckSnapshots.find((s) => s.id === bestSnap.id)?.label ?? `Snapshot #${bestSnap.id}`;
    ...
    if (deckStats && deckStats.totalGamesAnalyzed >= 3) {
      ...
      if (deckStats.prizeEfficiency.lossGamesCount >= 2
