...
const ROLE_OPTIONS: Record<CardType, { value: CardRole; label: string; hint: string }[]> = {
  Pokemon: [
    { value: 'attacker', label: 'Attacker', hint: 'Main damage dealer' },
    { value: 'tech',     label: 'Tech',     hint: 'Single-copy answer' },
  ],
  Trainer: [
    { value: 'supporter', label: 'Supporter', hint: 'Supporter-rule card' },
    { value: 'item',      label: 'Item',      hint: 'Item card' },
    { value: 'stadium',   label: 'Stadium',   hint: 'Stadium card' },
    { value: 'tech',      label: 'Tech',      hint: 'Single-copy answer' },
  ],
  Energy: [
    { value: 'energy', label: 'Energy', hint: 'Energy card' },
  ],
};
...
const COLUMN_STYLES: Record<CardType, { header: string; accent: string }> = {
  Pokemon: { header: 'text-red-300 border-red-900/40 bg-red-950/30',    accent: 'focus:border-red-700' },
  Trainer: { header: 'text-blue-300 border-blue-900/40 bg-blue-950/30', accent: 'focus:border-blue-700' },
  Energy:  { header: 'text-orange-300 border-orange-900/40 bg-orange-950/30', accent: 'focus:border-orange-700' },
};

// ─── Autocomplete suggestions ─────────────────────────────────────────────────

const KNOWN_POKEMON = [
  'Dragapult ex', 'Dragapult', 'Duskull', 'Dusclops', 'Dusknoir',
  'Lucario ex', 'Lucario', 'Riolu', 'Hariyama', 'Makuhita',
  "N's Zorua", "N's Zoroark ex", "N's Zoroark", 'Zorua', 'Zoroark',
  'Froslass', 'Snorunt', 'Grimmsnarl ex', 'Grimmsnarl', 'Impidimp', 'Morgrem',
  'Ogerpon ex', 'Teal Mask Ogerpon ex', 'Wellspring Mask Ogerpon ex', 'Cornerstone Mask Ogerpon ex',
  'Meganium ex', 'Chikorita', 'Bayleef', 'Raging Bolt ex',
  'Starmie ex', 'Staryu', 'Alakazam ex', 'Kadabra', 'Abra',
  'Dudunsparce', 'Dunsparce', 'Absol',
  'Kangaskhan ex', 'Kangaskhan', "Rocket's Mewtwo ex", 'Mewtwo ex',
  "Cynthia's Garchomp ex", 'Garchomp', 'Gible', 'Gabite',
  'Okidogi ex', 'Barbaracle', 'Binacle',
  'Greninja ex', 'Froakie', 'Frogadier',
  'Venusaur ex', 'Ivysaur', 'Bulbasaur',
  "Steven's Metagross ex", 'Metagross', 'Beldum', 'Metang',
  "Rocket's Honchkrow ex", 'Honchkrow', 'Murkrow',
  'Mega Lucario ex', 'Lucario ex', 'Ceruledge ex', 'Charcadet',
  'Mega Starmie ex', 'Decidueye ex', 'Dartrix', 'Rowlet',
  'Hydreigon ex', 'Zweilous', 'Deino',
  'Charizard ex', 'Charmeleon', 'Charmander',
  'Archaludon ex', 'Duraludon', "Hop's Zacian ex", 'Zacian',
  "Ethan's Typhlosion ex", 'Typhlosion', 'Quilava', 'Cyndaquil',
  'Terapagos ex', 'Jellicent ex', 'Frillish',
  "Rocket's Spidops ex", 'Tarountula',
  "Hop's Trevenant ex", 'Phantump',
  'Ursaluna ex', 'Ursaring', 'Teddiursa', 'Lunatone',
  'Lopunny ex', 'Buneary', 'Hydrapple ex', 'Dipplin', 'Applin',
  'Yanmega ex', 'Yanma', 'Zygarde ex', 'Bouffalant',
  'Clefairy', 'Clefable', 'Crustle', 'Dwebble',
  'Slowking ex', 'Slowpoke', 'Flareon ex', 'Eevee', 'Noctowl', 'Hoothoot',
  'Munkidori', 'Diancie ex', 'Archaludon', 'Kangaskhan-Mega ex',
  "Marnie's Grimmsnarl ex",
];

const KNOWN_TRAINERS = [
  // Supporters
  "Professor's Research", 'Professorin Forschung',
  'Iono', 'Arven', "Boss's Orders", 'Befehl des Bosses',
  'N', 'Cynthia', 'Colza', 'Kieran', 'Janine',
  'Rocket Boss Giovanni', 'Katy', 'Hop', 'Marnie', 'Steven',
  'Lt. Surge', 'Ethan', 'Tord',
  // Items
  'Nest Ball', 'Nestball',
  'Ultra Ball', 'Hyperball',
  'Rare Candy', 'Sonderbonbon',
  'Buddy-Buddy Poffin', 'Dicke-Freunde-Knursp',
  'Escape Rope', 'Fluchtnetz',
  'Switch', 'Switching Cart',
  'Lost Vacuum', 'Leeresog',
  'Earthen Vessel', 'Erdbehälter',
  "Hero's Cape", 'Heldenumhang',
  'Technical Machine: Evolution', 'Technical Machine: Devolution',
  'Pal Pad', 'Rescue Board',
  'Energy Recycler', 'Night Stretcher',
  'Super Rod', 'Counter Catcher',
  'Pokégear 3.0', 'Fog Crystal',
  'Iron Valiant ex', 'Iron Hands ex',
  'Ancient Booster Energy Capsule',
  'Collapsed Stadium', 'Zusammengebrochenes Stadion',
  'Path to the Peak', 'Weg zur Spitze',
  'Primeval Forest', 'Ewiger Urwald',
  'Rocky Helmet', 'Artazon',
];

const KNOWN_ENERGY = [
  'Grass Energy', 'Grasenergie',
  'Fire Energy', 'Feuerenergie',
  'Water Energy', 'Wasserenergie',
  'Lightning Energy', 'Blitzenergie',
  'Psychic Energy', 'Psychoenergie',
  'Fighting Energy', 'Kampfenergie',
  'Darkness Energy', 'Dunkelenergie',
  'Metal Energy', 'Metallenergie',
  'Dragon Energy', 'Drachenenergie',
  'Double Turbo Energy',
  'Reversal Energy', 'Umkehrenergie',
  'Jet Energy', 'Therapeutic Energy',
...
function RoleDropdown({
  ...
  role: CardRole; type: CardType; onChange: (r: CardRole) => void;
}) {
...
function CountStepper({
  ...
  count: number; onChange: (n: number) => void; onRemove: () => void;
}) {
  const dec = () => { if (count <= 1) onRemove(); else onChange(count - 1); };
  const inc = () => { if (count < 4) onChange(count + 1); };
  ...
  return (
    <div className="flex items-center gap-0.5 bg-gray-800/70 border border-gray-700 rounded">
      ...
      >
        {count <= 1 ? <Trash2 className="w-2.5 h-2.5" /> : <Minus className="w-3 h-3" />}
      </button>
...
function CardRow({
  card, onChangeCount, onChangeRole, onDelete,
}: {
  card: DeckCard;
...
function CardNameCombobox({
  type, deckCards, onSubmit, placeholder, accent,
}: {
  type: CardType;
  deckCards: DeckCard[];
...
function CardColumn({
  type, cards, allDeckCards, onChangeCount, onChangeRole, onDelete, onQuickAdd,
}: {
  type: CardType;
  cards: DeckCard[];
...
export function DeckPanel({ deckCards }: Props) {
  ...
  const handleQuickAdd = async (type: CardType, name: string) => {
    const defaultRole: CardRole = type === 'Pokemon' ? 'attacker' : type === 'Trainer' ? 'item' : 'energy';
    const existing = deckCards.find((c) => c.name === name && c.type === type);

    if (existing?.id != null) {
      const newCount = Math.min(existing.count + 1, 4);
      patchDeckCards((cards) => cards.map((c) => c.id === existing.id ? { ...c, count: newCount } : c));
      try {
    ...
    const newCard: DeckCard = {
      id: tempId, name, count: 1, type, role: defaultRole,
      cardId: 0, deckId: activeDeckId ?? 0,
    };
    ...
    <div className="card overflow-hidden p-0">
      ...
      <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-gray-800">
        <div>
          ...
          <p className="text-xs text-gray-500 mt-0.5">
            {totalCards}/60 cards · click role to change · use <kbd className="px-1 rounded bg-gray-800 border border-gray-700 text-[10px] font-mono">+</kbd>/<kbd className="px-1 rounded bg-gray-800 border border-gray-700 text-[10px] font-mono">−</kbd> to adjust counts
          </p>
        </div>
        <button
      ...
      <div className="px-4 py-2 border-b border-gray-800">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-xs font-mono shrink-0 ${totalCards === 60 ? 'text-emerald-400' : totalCards > 60 ? 'text-red-400' : 'text-gray-500'}`}>
            {totalCards}/60
          </span>
        </div>
      </div>
      ...
      <div className="flex border-b border-gray-800 md:hidden">
        ...
        ] as [CardType, typeof pokemon][]).map(([type, cards]) => {
      ...
      <div className="md:hidden">
        {mobileTab === 'Pokemon' && (
          <CardColumn type="Pokemon" cards={pokemon} allDeckCards={deckCards} onChangeCount={handleChangeCount} onChangeRole={handleChangeRole} onDelete={handleDelete} onQuickAdd={(name) => handleQuickAdd('Pokemon', name)} />
        )}
        {mobileTab === 'Trainer' && (
          <CardColumn type="Trainer" cards={trainers} allDeckCards={deckCards} onChangeCount={handleChangeCount} onChangeRole={handleChangeRole} onDelete={handleDelete} onQuickAdd={(name) => handleQuickAdd('Trainer', name)} />
        )}
        {mobileTab === 'Energy' && (
          <CardColumn type="Energy" cards={energies} allDeckCards={deckCards} onChangeCount={handleChangeCount} onChangeRole={handleChangeRole} onDelete={handleDelete} onQuickAdd={(name) => handleQuickAdd('Energy', name)} />
        )}
      </div>
      ...
      {showImportModal && <ImportDeckModal onClose={() => { setShowImportModal(false); refresh(); }} />}
    </div>
  );
}
