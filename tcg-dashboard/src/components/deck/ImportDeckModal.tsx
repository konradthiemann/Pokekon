...
function CardGroup({ label, cards, total }: { label: string; cards: ParsedCard[]; total: number }) {
  ...
  const color = TYPE_COLOR[label === 'Pokémon' ? 'Pokemon' : label === 'Trainers' ? 'Trainer' : 'Energy'] ?? 'text-gray-400';
      ...
      {open && (
        <div>
          {cards.map((card, i) => (
            ...
            >
              <span className="text-white/40 text-xs w-4 font-mono text-right shrink-0">{card.count}</span>
              ...
              <span className={`badge border text-[10px] shrink-0 ${ROLE_COLORS[card.role] ?? 'bg-gray-800 text-gray-400 border-gray-700'}`}>
                {card.role}
              </span>
            </div>
          ))}
      ...
      ">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-800 shrink-0">
          <div className="flex items-center gap-2">
            ...
            {step === 'preview' && (
              <span className={`text-xs font-semibold ml-1 ${totalCount === 60 ? 'text-emerald-400' : 'text-yellow-400'}`}>
                · {totalCount}/60
              </span>
            )}
          </div>
          ...
          {step === 'paste' && (
            <>
              <p className="text-xs text-gray-400">
                Paste a deck list with <span className="text-gray-300">Pokémon / Trainer / Energie</span> sections.
                Each line: <code className="text-brand-400">count name SET number</code>
              </p>
              <textarea
                value={text}
          ...
          {step === 'preview' && (
            <>
              ...
              <div>
                ...
                {replace && (
                  <p className="text-xs text-yellow-500 pl-5">Alle vorhandenen Karten werden gelöscht.</p>
                )}
                {activeDeckId == null && (
              ...
              {skipped.length > 0 && (
                <div className="p-3 bg-yellow-900/20 border border-yellow-800/40 rounded-lg">
                  ...
                  {skipped.map((line, i) => (
                    <p key={i} className="text-xs text-yellow-600/80 font-mono truncate">{line}</p>
                  ))}
                </div>
              )}
        ...
        <div className="flex gap-2 px-5 py-4 border-t border-gray-800 shrink-0">
          {step === 'paste' && (
            <>
              <button onClick={onClose} className="btn-ghost flex-1 justify-center text-sm">Abbrechen</button>
              <button
                onClick={handleParse}
                disabled={!text.trim()}
          ...
          {step === 'preview' && (
            <>
              <button onClick={() => setStep('paste')} className="btn-ghost flex-1 justify-center text-sm">
                Zurück
              </button>
              <button
          ...
          {step === 'done' && (
            <button onClick={onClose} className="btn-primary flex-1 justify-center text-sm">Fertig</button>
          )}
        </div>
      </div>
    </div>
