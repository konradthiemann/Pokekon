...
import { BarChart2, TrendingUp, TrendingDown, Target, Zap, GitCompare, Shield, AlertTriangle, Layers } from 'lucide-react';
...
function wilsonLower(wins: number, n: number): number {
  ...
  const z = 1.96;
  const denom = 1 + z * z / n;
  return Math.round(((p + z * z / (2 * n) - z * Math.sqrt((p * (1 - p) + z * z / (4 * n)) / n)) / denom) * 100);
}
...
function WrBar({ value, max = 100 }: { value: number; max?: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className={`text-xs tabular-nums w-8 text-right font-semibold ${value >= 60 ? 'text-emerald-400' : value >= 45 ? 'text-yellow-400' : 'text-red-400'}`}>
        {value}%
      </span>
    </div>
  );
...
function computeDeckStats(deck: Deck, logs: OpponentLog[], freqMap: Map<string, number>): DeckStats {
  ...
  const matchups = [...statsMap.entries()].map(([archetype, s]) => {
    ...
    return {
      ...
      tier: rate >= 58 ? 'favorable' as const : rate >= 42 ? 'even' as const : 'unfavorable' as const,
    };
...
function VariantComparison({ variants }: { variants: DeckStats[] }) {
  ...
  const allMatchups = [...new Set(
    variants.flatMap((v) => v.matchups.filter((m) => m.wins + m.losses >= 2).map((m) => m.archetype))
  )];
        ...
        <table className="w-full text-xs">
          <thead>
            <tr className="border-b border-gray-800 bg-gray-800/30">
              ...
              {variants.map((v) => (
                <th key={v.deck.id} className="px-3 py-2.5 text-center text-gray-400 font-medium min-w-[90px]">
                  {v.deck.variant}
                </th>
              ))}
            ...
            <tr className="border-b border-gray-800 bg-gray-800/20">
              ...
              {variants.map((v) => (
                <td key={v.deck.id} className="px-3 py-2.5 text-center">
                  <span className={`font-semibold ${v.winRate >= 55 ? 'text-emerald-400' : v.winRate >= 45 ? 'text-gray-200' : 'text-red-400'}`}>
                    {v.games > 0 ? `${v.winRate}%` : '—'}
                  </span>
              ...
              <td className="px-3 py-2.5 text-center">
                {variants.length >= 2 && variants[0].games > 0 && variants[1].games > 0 && (() => {
                  const best = variants.reduce((a, b) => a.winRate > b.winRate ? a : b);
                  const diff = Math.max(...variants.map((v) => v.winRate)) - Math.min(...variants.map((v) => v.winRate));
                  return diff >= 3
                    ? <span className="text-brand-400 font-medium">{best.deck.variant} +{diff}%</span>
                ...
                <tr key={arch} className="border-b border-gray-800/40 hover:bg-gray-800/10">
                  ...
                  {cells.map((c, i) => (
                    <td key={variants[i].deck.id} className="px-3 py-2 text-center">
                      {c && (c.wins + c.losses) >= 2 ? (
                        <span className={`font-semibold ${c.winRate >= 58 ? 'text-emerald-400' : c.winRate >= 42 ? 'text-gray-300' : 'text-red-400'}`}>
                          {c.winRate}%
                          <span className="text-gray-600 font-normal ml-1 text-xs">({c.wins}-{c.losses})</span>
                        </span>
                      ) : (
  ...
  return (
    <div className="space-y-1">
      {sorted.map((m) => {
        ...
        return (
          <div key={m.archetype} className={`flex items-center gap-3 px-1 py-1.5 rounded hover:bg-gray-800/30 transition-colors ${isLowData ? 'opacity-60' : ''}`}>
            ...
            <div className="w-16 shrink-0">
              {isLowData ? (
                ...
                <span className={`text-xs px-1.5 py-0.5 rounded ${
                  m.tier === 'favorable' ? 'bg-emerald-900/40 text-emerald-400' :
                  m.tier === 'unfavorable' ? 'bg-red-900/40 text-red-400' :
                  'bg-gray-800/40 text-gray-400'
                }`}>
                  {m.tier === 'favorable' ? 'Favored' : m.tier === 'unfavorable' ? 'Unfav.' : 'Even'}
                </span>
              )}
            </div>
          </div>
...
function MetaInsights({ stats, metaSnapshots }: { stats: DeckStats; metaSnapshots: MetaSnapshot[] }) {
  ...
  const coverage = topMeta.filter((m) => {
    const match = stats.matchups.find((mu) => mu.archetype.toLowerCase().includes(m.archetype.toLowerCase().split('-')[0]));
  ...
  const gatekeeper = stats.matchups.find((m) => {
    const isTop3 = topMeta.slice(0, 3).some((t) =>
      m.archetype.toLowerCase().includes(t.archetype.toLowerCase().split('-')[0])
    );
      ...
      {gatekeeper && (
        <div className="flex items-start gap-2 p-2.5 bg-red-900/20 border border-red-900/40 rounded-lg text-xs">
          ...
          <div>
            ...
            <span className="text-gray-300 ml-1">{gatekeeper.archetype} ({gatekeeper.winRate}% WR) is a top-3 meta deck you're losing to. Improve this matchup first.</span>
          </div>
        </div>
      )}
      ...
      {coveragePct < 50 && topMeta.length > 0 && (
        <div className="flex items-start gap-2 p-2.5 bg-yellow-900/20 border border-yellow-900/40 rounded-lg text-xs">
          ...
          <div>
            ...
            <span className="text-gray-300 ml-1">You have data on {coveragePct}% of top meta decks. Log more matches for reliable meta score.</span>
          </div>
        </div>
      )}
      ...
      {bestMatchup && (
        <div className="flex items-start gap-2 p-2.5 bg-emerald-900/20 border border-emerald-900/40 rounded-lg text-xs">
          ...
          <span className="text-gray-300">
            <span className="text-emerald-300 font-medium">Best matchup:</span> {bestMatchup.archetype} ({bestMatchup.winRate}% in {bestMatchup.wins + bestMatchup.losses + bestMatchup.ties} games)
          </span>
        </div>
      )}

      {worstMatchup && worstMatchup.archetype !== bestMatchup?.archetype && worstMatchup.winRate < 45 && (
        <div className="flex items-start gap-2 p-2.5 bg-gray-800/40 border border-gray-700/40 rounded-lg text-xs">
          ...
          <span className="text-gray-300">
            <span className="text-gray-200 font-medium">Worst matchup:</span> {worstMatchup.archetype} ({worstMatchup.winRate}% in {worstMatchup.wins + worstMatchup.losses + worstMatchup.ties} games)
          </span>
        </div>
      )}
    </div>
        ...
        <div className="card p-0 overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-800 flex items-center gap-2">
            ...
            <h3 className="card-header mb-0">
              {activeStats.deck.archetypeName} — Overall
            </h3>
            <span className="text-xs text-gray-500 ml-1">
              · {archetypeAggregate.variantCount} {archetypeAggregate.variantCount === 1 ? 'variant' : 'variants'} combined
            </span>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-gray-800">
            <div className="p-4 text-center">
              <div className={`text-2xl font-bold ${archetypeAggregate.winRate >= 55 ? 'text-emerald-400' : archetypeAggregate.winRate >= 45 ? 'text-yellow-400' : archetypeAggregate.games > 0 ? 'text-red-400' : 'text-gray-600'}`}>
                {archetypeAggregate.games > 0 ? `${archetypeAggregate.winRate}%` : '—'}
              </div>
              <div className="text-xs text-gray-500 mt-0.5">Overall WR</div>
            </div>
            <div className="p-4 text-center">
              <div className="text-2xl font-bold text-gray-200 tabular-nums">{archetypeAggregate.games}</div>
            ...
            <div className="p-4 text-center">
              ...
              <div className="text-xs text-gray-500 mt-0.5">
                Best Variant{archetypeAggregate.bestVariant && archetypeAggregate.bestVariant.games > 0 ? ` (${archetypeAggregate.bestVariant.winRate}%)` : ''}
              </div>
            </div>
          </div>
        </div>
      ...
      {activeStats && (
        <>
          <div className="flex items-center gap-2 text-xs text-gray-500 uppercase tracking-wider font-semibold pt-2">
            ...
            Variant: <span className="text-gray-300 normal-case font-medium tracking-normal">{activeStats.deck.variant || 'Default'}</span>
          </div>
          {/* KPI row */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div className="card p-4 text-center">
              ...
              <div className={`text-2xl font-bold ${activeStats.winRate >= 55 ? 'text-emerald-400' : activeStats.winRate >= 45 ? 'text-yellow-400' : 'text-red-400'}`}>
            ...
            <div className="card p-4 text-center">
              ...
              <div className={`text-2xl font-bold ${activeStats.metaScore >= 55 ? 'text-emerald-400' : activeStats.metaScore >= 45 ? 'text-yellow-400' : activeStats.metaScore > 0 ? 'text-red-400' : 'text-gray-600'}`}>
            ...
            <div className="card p-4 text-center">
              ...
              <div className={`text-2xl font-bold ${activeStats.recentWR >= 55 ? 'text-emerald-400' : activeStats.recentWR >= 45 ? 'text-yellow-400' : activeStats.recentWR > 0 ? 'text-red-400' : 'text-gray-600'}`}>
              ...
              <div className="flex items-center justify-center gap-0.5 mt-1">
                {activeStats.recentForm.map((r, i) => <FormDot key={i} r={r} />)}
              </div>
            </div>
            ...
            <div className="card p-4 text-center">
              ...
              <div className={`text-2xl font-bold ${activeStats.consistency >= 70 ? 'text-emerald-400' : activeStats.consistency >= 50 ? 'text-yellow-400' : 'text-red-400'}`}>
