...
export function MatchStatsTab({ data }: Props) {
  const { player1, player2, totalTurns, prizeProgression, damageByTurn, cardFrequency, totalDamage, totalKOs } = data;
        ...
        <section>
          ...
          <div className="bg-gray-800/30 rounded-lg border border-gray-700/40 p-3">
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={prizeProgression} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                ...
                />
                <YAxis
                  domain={[0, 6]} ticks={[0, 1, 2, 3, 4, 5, 6]}
                  tick={{ fill: '#6b7280', fontSize: 10 }}
                  axisLine={false} tickLine={false}
                />
                <Tooltip
                  contentStyle={C_TIP} labelStyle={C_LAB}
                ...
                />
                <Line
                  type="stepAfter" dataKey="p1" name={player1}
                  stroke={C_P1} strokeWidth={2} dot={false}
                />
                <Line
                  type="stepAfter" dataKey="p2" name={player2}
                  stroke={C_P2} strokeWidth={2} dot={false} strokeDasharray="4 2"
                />
              </LineChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="bg-gray-800/30 rounded-lg border border-gray-700/40 p-3">
            <ResponsiveContainer width="100%" height={160}>
              <BarChart data={damageByTurn} margin={{ top: 4, right: 8, left: -20, bottom: 0 }}>
                ...
                />
                <Bar dataKey="p1" name={player1} fill={C_P1} radius={[3, 3, 0, 0]} maxBarSize={28} />
                <Bar dataKey="p2" name={player2} fill={C_P2} radius={[3, 3, 0, 0]} maxBarSize={28} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </section>
          ...
          <div className="bg-gray-800/30 rounded-lg border border-gray-700/40 p-3">
            <ResponsiveContainer width="100%" height={Math.max(140, cardFrequency.length * 24)}>
              ...
              >
                ...
                />
                <YAxis
                  type="category" dataKey="card" width={160}
