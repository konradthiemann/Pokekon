...
import { X, FileText, Brain, ChevronDown, ChevronUp, AlertTriangle, TrendingUp, Layers, Save, RefreshCw, BarChart2 } from 'lucide-react';
...
function ImpactBadge({ impact }: { impact: 'high' | 'medium' | 'low' }) {
  const cls =
    impact === 'high'   ? 'bg-red-900/50 text-red-300 border border-red-700/50' :
    impact === 'medium' ? 'bg-yellow-900/50 text-yellow-300 border border-yellow-700/50' :
...
function ActionBadge({ action }: { action: 'add' | 'remove' | 'increase' | 'decrease' }) {
  ...
  const label = { add: '+hinzufügen', remove: '−entfernen', increase: '▲ erhöhen', decrease: '▼ reduzieren' };
  return <span className={`text-[10px] font-medium px-1.5 py-0.5 rounded ${map[action]}`}>{label[action]}</span>;
}
        ...
        <div className="px-3 pb-3 space-y-2 text-sm">
          ...
          {item.suggestion && (
            <div className="flex gap-1.5">
              <span className="text-[10px] text-brand-400 font-medium shrink-0 mt-0.5">Vorschlag:</span>
    ...
    <div className="space-y-5">
      ...
      <div className="bg-gray-800/40 rounded-lg px-4 py-3 border border-gray-700/40">
        ...
        <p className="text-[10px] text-gray-600 mt-2">
          Analysiert am {new Date(analysis.analyzedAt).toLocaleString('de-DE')} ·
          Spieler: {analysis.playerName} · Gegner: {analysis.opponentName}
        </p>
      </div>
      ...
      {analysis.playMistakes.length > 0 && (
        <section>
          ...
          <div className="space-y-1.5">
            {analysis.playMistakes.map((m, i) => <PlayCard key={i} item={m} />)}
          </div>
        </section>
      )}
      ...
      {analysis.keyMoments.length > 0 && (
        <section>
          ...
          <div className="space-y-1.5">
            {analysis.keyMoments.map((m, i) => <PlayCard key={i} item={m} />)}
          </div>
        </section>
      )}
        ...
        <section>
          ...
          <div className="space-y-1.5">
            {analysis.deckSuggestions.map((s, i) => (
              <div key={i} className="bg-gray-800/60 rounded-lg border border-gray-700/50 px-3 py-2.5 space-y-1.5">
        ...
        <section>
          ...
          <div className="space-y-1.5">
            {analysis.cardNotes.map((n, i) => (
              <div key={i} className="bg-gray-800/60 rounded-lg border border-gray-700/50 px-3 py-2.5 space-y-1">
                <div className="flex items-center gap-2">
...
export function MatchDetailModal({ log, onClose }: Props) {
  ...
  const storedAnalysis: BattleAnalysis | null = (() => {
    try { return log.analysis ? JSON.parse(log.analysis) : null; } catch { return null; }
  })();
  ...
  useEffect(() => {
    ...
    getDeckSnapshotById(log.deckSnapshotId).then((snap) => {
      if (!snap) { setSnapshotError(true); return; }
  ...
  const parsedStats = useMemo(() => {
    ...
    try { return parseBattleLog(battleLog, playerName); } catch { return null; }
  ...
  const resultBadge =
    log.result === 'W' ? 'badge-win' :
    log.result === 'L' ? 'badge-loss' : 'badge-tie';
  const resultLabel =
      ...
      <div className="bg-gray-900 border border-gray-700 rounded-t-2xl sm:rounded-xl w-full max-w-2xl max-h-[92vh] sm:max-h-[90vh] flex flex-col shadow-2xl">
        ...
        <div className="flex items-start justify-between px-5 pt-5 pb-4 border-b border-gray-800 shrink-0">
          <div>
            <div className="flex items-center gap-2.5 flex-wrap">
              ...
              <span className={log.eventType === 'LC' ? 'badge-lc' : 'badge-lcup'}>{log.eventType}</span>
            </div>
            <p className="text-xs text-gray-500 mt-1">
              {log.eventDate}{log.round ? ` · Runde ${log.round}` : ''}
              {log.notes ? ` · ${log.notes}` : ''}
            </p>
          </div>
        ...
        <div className="flex border-b border-gray-800 shrink-0 overflow-x-auto scrollbar-none">
          ...
          >
            ...
            KI-Analyse
            {analysis && <span className="ml-1 w-1.5 h-1.5 rounded-full bg-green-500 inline-block" />}
          </button>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 min-h-0">
          ...
          {activeTab === 'log' && (
            <div className="space-y-3">
              <p className="text-xs text-gray-500">
                Füge das Kampfprotokoll aus PTCGL / PTCGO ein. Es wird lokal gespeichert und für die KI-Analyse verwendet.
              </p>
              <textarea
                value={battleLog}
                onChange={(e) => { setBattleLog(e.target.value); setLogDirty(true); }}
          ...
          {activeTab === 'snapshot' && (
            <div>
              {snapshotError && (
                <p className="text-sm text-gray-500 text-center py-10">Snapshot nicht mehr vorhanden.</p>
              )}
              {!snapshotError && !snapshotCards && (
                <p className="text-sm text-gray-600 text-center py-10">Lade…</p>
              )}
              {snapshotCards && (() => {
                const grouped: Record<string, DeckCard[]> = { Pokemon: [], Trainer: [], Energy: [] };
                ...
                return (
                  <div className="space-y-4">
                    {(['Pokemon', 'Trainer', 'Energy'] as const).map((type) => {
                      ...
                      const color = type === 'Pokemon' ? 'text-red-400' : type === 'Trainer' ? 'text-blue-400' : 'text-orange-400';
                      return (
                        <section key={type}>
                          ...
                          <div className="space-y-0.5">
                            {cards.map((c) => (
                              <div key={c.name} className="flex items-center gap-2 text-sm py-1 px-2 rounded hover:bg-gray-800/40">
                                <span className="text-gray-500 tabular-nums w-5 text-right shrink-0">{c.count}×</span>
          ...
          {activeTab === 'analysis' && (
            <div className="space-y-4">
              ...
              <div className="bg-gray-800/40 rounded-lg border border-gray-700/40 p-4 space-y-3">
                <h3 className="text-xs font-medium text-gray-400 uppercase tracking-wide">Analyse-Einstellungen</h3>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs text-gray-500 mb-1">Mein Spielername im Protokoll</label>
                    <input
                      type="text"
                      value={playerName}
                ...
                >
                  {analyzing
                    ...
                    : <><Brain className="w-3.5 h-3.5" />{analysis ? 'Neu analysieren' : 'Analyse starten'}</>
                  }
                </button>
                {analysisError && (
              ...
              {!analysis && !analyzing && !analysisError && (
                <p className="text-sm text-gray-600 text-center py-8">
                  Noch keine Analyse vorhanden. Gib deinen Spielernamen und API-Key ein und starte die Analyse.
                </p>
              )}
            </div>
          )}
