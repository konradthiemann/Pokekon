...
const RESULT_BUTTONS = [
  ['W', 'Win',  'bg-emerald-600 hover:bg-emerald-500 border-emerald-500 text-white', 'bg-emerald-950/40 border-emerald-800 text-emerald-400'],
  ['L', 'Loss', 'bg-red-600 hover:bg-red-500 border-red-500 text-white',             'bg-red-950/40 border-red-800 text-red-400'],
  ['T', 'Tie',  'bg-yellow-600 hover:bg-yellow-500 border-yellow-500 text-white',    'bg-yellow-950/40 border-yellow-800 text-yellow-400'],
] as const;
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/70">
      <div className="bg-gray-900 border border-gray-700 rounded-t-2xl sm:rounded-xl p-5 sm:p-6 w-full max-w-md shadow-xl max-h-[92vh] overflow-y-auto">
            ...
            >
              ...
              {decks.map((d) => (
                <option key={d.id} value={d.id}>
                  {d.archetypeName}{d.variant ? ` · ${d.variant}` : ''}
                </option>
              ))}
            </select>
          </div>
            ...
            <div className="grid grid-cols-2 gap-1.5">
              {KNOWN_ARCHETYPES.map((a) => (
                <button
                  key={a.slug}
                  type="button"
                  onClick={() => { setArchetype(a.slug); setCustomArch(''); }}
              ...
              {relevantSnapshots.length > 0 && (
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setDeckSnapshotId(e.target.value === '' ? '' : Number(e.target.value))}
                    className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-brand-500"
                  >
        ...
        <div className="flex gap-2 mt-4">
          <button onClick={onClose} className="btn-ghost justify-center text-sm px-4">Cancel</button>
          <button
            onClick={handleSave}
