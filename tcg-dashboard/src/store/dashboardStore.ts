...
import type { Deck, DeckCard, DeckSnapshot, OpponentLog, MetaSnapshot, ArchetypeStats, RecentTournament } from '../types';
import {
  getDeckCards,
  getOpponentLogs,
...
import {
  ...
  getDeckArchSlug, setDeckArchSlug,
  getActiveDeckId, setActiveDeckId,
...
interface DashboardState {
  ...
  loadRecentTournaments: (opts?: { days?: number; minPlayers?: number; onlineOnly?: boolean }) => Promise<void>;
  ...
  duplicateDeckAsVariant: (variantName: string, opts?: { copyCards?: boolean }) => Promise<number | null>;
...
export const useDashboardStore = create<DashboardState>((set, get) => ({
  ...
  refresh: async () => {
    ...
    try {
      ...
      const [deckCards, deckSnapshots, opponentLogs, metaSnapshots, archetypeStats] = await Promise.all([
  ...
  loadRecentTournaments: async (opts) => {
    ...
    try {
      ...
      set({ isFetchingTournaments: false, tournamentsError: err instanceof Error ? err.message : 'Failed' });
    }
  },
  ...
  runDeckComparison: async () => {
    ...
    try {
      const result = await fetchArchetypeComparison(
        slug,
        deckCards,
        (msg) => set({ compareProgress: msg }),
      );
      set({ comparisonResult: result, isComparing: false, compareProgress: '' });
    } catch (err) {
      set({ isComparing: false, compareProgress: '', compareError: err instanceof Error ? err.message : 'Comparison failed' });
    }
  },
}));
