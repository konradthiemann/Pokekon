...
function DeckSettingsWidget() {
  ...
  const handleSave = async () => {
    await updateCurrentDeck({ archetype: archetype.trim(), archetypeName: archetypeName.trim(), variant: variant.trim() });
    setSaved(true);
  ...
  return (
    <SidePanel icon={<Settings2 className="w-4 h-4" />} title="Deck Settings" description="Rename this deck or duplicate it as a new variant.">
      <div className="flex flex-col gap-4 h-full">
        <div className="space-y-2">
          <div>
            <label className="block text-[11px] uppercase tracking-wider text-white/40 mb-1 font-medium">Archetype</label>
          ...
          <div>
            <label className="block text-[11px] uppercase tracking-wider text-white/40 mb-1 font-medium">Variant label</label>
          ...
          {showAdvanced ? (
            <div>
              <label className="block text-[11px] uppercase tracking-wider text-white/40 mb-1 font-medium">Limitless slug</label>
              ...
              <p className="text-[11px] text-white/25 mt-1">Lowercase, hyphen-separated. Matches against Limitless meta data.</p>
            </div>
          ) : (
            <button onClick={() => setShowAdvanced(true)} className="text-[11px] text-white/30 hover:text-brand-300 transition-colors">
              + Advanced (Limitless slug)
            </button>
          )}
          <button onClick={handleSave} disabled={!archetypeName.trim() || !dirty}
        ...
        <div className="space-y-2">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[11px] uppercase tracking-wider text-white/40 font-medium">New variant of this deck</span>
          </div>
          <p className="text-[11px] text-white/25 leading-snug">
            Copies all cards into a new {activeDeck.archetypeName} variant so you can tweak without losing this one.
          </p>
          <div className="flex gap-2">
            ...
            <button onClick={handleCreateVariant} disabled={!newVariantName.trim() || copying}
...
export function DeckPage() {
  const { decks, activeDeckId, deckCards, opponentLogs, metaSnapshots, activeDeck } = useDashboardStore();
