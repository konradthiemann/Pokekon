...
export function RecommendationsPage() {
  const { deckCards, archetypeStats, opponentLogs, deckSnapshots, localMeta, activeDeckId, activeDeck } = useDashboardStore();
  ...
  const deckStats = useMemo(() => computeDeckPerformanceStats(activeLogs, playerName), [activeLogs, playerName]);
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-500 text-sm">
          {activeDeck
            ? <>Für: <span className="text-gray-300 font-medium">{activeDeck.archetypeName}</span> — basierend auf deinen Matches und dem aktuellen Meta</>
      ...
      <div className="flex items-start gap-3 bg-gray-800/50 border border-gray-700 rounded-xl p-4">
        ...
        <div className="text-sm text-gray-400 space-y-1">
          <div>
            Based on{' '}
            <span className="text-white font-medium">{activeLogs.length} logged matches</span>
            {' '}and current meta data.{' '}
      ...
      {recommendations.length > 0 && (
        <div className="flex items-center gap-4 text-xs text-gray-500">
          <span><span className="text-red-400 font-medium">{highCount}</span> high priority</span>
          <span><span className="text-yellow-400 font-medium">{medCount}</span> medium priority</span>
          <span><span className="text-blue-400 font-medium">{lowCount}</span> low priority</span>
        </div>
      )}
      ...
      <div className="border-t border-gray-800 pt-2">
        <h2 className="text-base font-semibold text-white mb-4">List Comparison vs. Tournament Results</h2>
        <DeckComparisonPanel />
      </div>
    </div>
  );
